...
import { Heart, ShoppingCart, Trash2, Share2, ChevronRight } from 'lucide-react';
...
export default function WishlistPage() {
  ...
  const removeItem = (id: string) => setItems(items.filter((p) => p.id !== id));
                  ...
                  <div className="space-y-1">
                    {categories.map((cat) => (
                      <label key={cat} className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="ml-auto text-xs text-gray-400">({items.filter((p) => cat === 'All' || p.category === cat).length})</span>
                      </label>
                    ))}
                    <button onClick={() => { setPriceRange([0, 500]); setSelectedCategory('All'); }} className="text-sm text-blue-600 hover:underline mt-2">Clear Filters</button>
                  </div>
                </div>
              </div>
            </aside>
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                {items.length > 0 && (
                  <div className="flex gap-3">
                    ...
                    <button className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
