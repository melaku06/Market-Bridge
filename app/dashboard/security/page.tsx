...
export default function SecurityPage() {
  ...
  const sessions = [
    { device: 'Chrome on Windows', location: 'New York, USA', time: 'Current Session', current: true, icon: <Monitor style={{ width: 14, height: 14 }} /> },
    { device: 'Safari on iPhone 14', location: 'Los Angeles, USA', time: 'May 18, 2024 at 9:30 AM', current: false, icon: <Smartphone style={{ width: 14, height: 14 }} /> },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 p-5">
          ...
          <form onSubmit={handleSave} className="space-y-3">
            <div>
              ...
              <div className="relative">
                <input type={showCurrent ? 'text' : 'password'} defaultValue="••••••••" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-[13px] focus:border-blue-500 focus:outline-none transition-all pr-10 bg-gray-50 focus:bg-white" />
            ...
            <div>
              ...
              <div className="relative">
                <input type={showNew ? 'text' : 'password'} placeholder="Create new password" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-[13px] focus:border-blue-500 focus:outline-none transition-all pr-10 bg-gray-50 focus:bg-white" />
            ...
            <div>
              ...
              <div className="relative">
                <input type={showConfirm ? 'text' : 'password'} placeholder="Confirm new password" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-[13px] focus:border-blue-500 focus:outline-none transition-all pr-10 bg-gray-50 focus:bg-white" />
          ...
          <div className="mt-5 pt-5 border-t border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[13px] font-semibold text-gray-900">Two-Factor Authentication</p>
                <p className="text-[11px] text-gray-500 mt-0.5">Add an extra layer of security to your account.</p>
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${twoFAEnabled ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
                  {twoFAEnabled ? 'Enabled' : 'Disabled'}
                </span>
                <button onClick={() => setTwoFAEnabled(!twoFAEnabled)} className="text-[12px] text-blue-600 hover:text-blue-700 font-medium border border-blue-200 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition-colors">
          ...
          <div className="mt-5 pt-5 border-t border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[13px] font-semibold text-gray-900">Active Sessions</p>
              <p className="text-[11px] text-gray-500">You are currently signed in on these devices.</p>
            </div>
              ...
              {sessions.map((session, i) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 hover:bg-gray-50/50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-medium text-gray-900">{session.device}</p>
                    <p className="text-[11px] text-gray-500">{session.location} • {session.time}</p>
                  </div>
                  {session.current ? (
                    <span className="text-[10px] bg-emerald-50 text-emerald-700 px-2 py-1 rounded-full font-medium">This Device</span>
                  ) : (
                    <button className="text-[11px] text-red-600 hover:text-red-700 font-medium border border-red-200 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors">
                      Sign Out
                    </button>
                  )}
                </div>
            ...
            <button className="mt-3 w-full text-[12px] text-red-600 hover:text-red-700 font-medium flex items-center justify-center gap-1.5 py-2 border border-red-200 rounded-lg hover:bg-red-50 transition-colors">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-amber-50 rounded-xl border border-amber-200 p-4">
            ...
            <button className="mt-3 text-[12px] font-semibold text-amber-700 hover:text-amber-800 underline">
              Contact Support
            </button>
          </div>
        </div>
